"""Build pooled top-150 local-income panels and rent-growth screens.

This workflow uses ACS1 metro-native annual household-income measures that
already exist in the curated ACS1 artifacts. It focuses on direct local income
levels rather than mover-income ratios, using the standard ACS1 lag rule:
ACS1 vintage E is aligned to PIT year E + 1.
"""
...
def load_acs1_local_income_panel() -> pd.DataFrame:
    ...
    for file_name in files:
        ...
        frame = pd.read_parquet(
            file_name,
            columns=["metro_id", "acs1_vintage", *LOCAL_INCOME_COLUMNS],
        )
        frame["msa_id"] = _as_msa_id(frame["metro_id"])
        frame["acs1_vintage_used"] = vintage
        frame["year"] = vintage + 1
        frames.append(frame[["msa_id", "year", "acs1_vintage_used", *LOCAL_INCOME_COLUMNS]])
...
def add_first_differences(levels: pd.DataFrame) -> pd.DataFrame:
    ...
    for column in LOCAL_INCOME_COLUMNS:
        log_column = f"log_{column}"
        levels[log_column] = _safe_log(levels[column])
        levels[f"d_{log_column}"] = grouped[log_column].diff()
    return levels


def build_levels_panel() -> pd.DataFrame:
    base = load_pooled_base_panel()
    income = load_acs1_local_income_panel()
...
def _model_specs() -> Iterable[ModelSpec]:
    for column in LOCAL_INCOME_COLUMNS:
        diff_column = f"d_log_{column}"
        yield ModelSpec(
            name=f"rent_fd_log_{column}",
            outcome="d_log_zori",
            predictors=("d_log_pop", diff_column),
        )
        yield ModelSpec(
            name=f"unsheltered_fd_log_{column}",
...
def summarize_panel(levels: pd.DataFrame, fd: pd.DataFrame) -> dict[str, object]:
    main_column = "median_household_income_by_tenure_total"
    complete_fd = fd.dropna(
        subset=["d_log_zori", "d_log_unshelt_rate", "d_log_pop", f"d_log_{main_column}"]
    )
    level_summary = (
        levels[LOCAL_INCOME_COLUMNS]
    ...
    correlations = (
        complete_fd[
            [
                ...
                "d_log_unshelt_rate",
                "d_log_pop",
                *[f"d_log_{column}" for column in LOCAL_INCOME_COLUMNS],
            ]
        ]
        .corr()
        .round(6)
    ...
    return {
        ...
        "complete_fd_rows_for_total_median_income": int(len(complete_fd)),
        "level_income_summary": json.loads(level_summary.to_json()),
        "levels_missing_income_columns": {
            column: int(levels[column].isna().sum()) for column in LOCAL_INCOME_COLUMNS
        },
        "fd_missing_income_growth_columns": {
            f"d_log_{column}": int(fd[f"d_log_{column}"].isna().sum())
            for column in LOCAL_INCOME_COLUMNS
        },
...
def main() -> None:
    ...
    levels_path = OUT / "local_income_composition_levels.parquet"
    fd_path = OUT / "local_income_composition_fd.parquet"
    regression_path = OUT / "local_income_composition_fd_regressions.parquet"
    regression_csv_path = OUT / "local_income_composition_fd_regressions.csv"
    summary_path = OUT / "local_income_composition_summary.json"
    ...
    if not regressions.empty:
        print(regressions[regressions["term"].str.contains("median_household_income")])
