...
def generate_text_summary(
    ...
    lines.append(f"  Mean coverage (across all CoCs):   {_format_stat(cov_mean.mean())}")
    ...
    lines.append(f"  CoCs with >= 99% coverage:   {_format_count_share(full_coverage, n_cocs)}")

    good_coverage = (cov_mean >= min_coverage).sum()
    lines.append(
        f"  CoCs with >= {min_coverage:.0%} coverage:  {_format_count_share(good_coverage, n_cocs)}"
    )

    low_coverage = (cov_mean < min_coverage).sum()
    lines.append(
        f"  CoCs with < {min_coverage:.0%} coverage:   {_format_count_share(low_coverage, n_cocs)}"
    )
    lines.append("")
