...
def parse_pep_county(raw_path: Path, vintage: int) -> pd.DataFrame:
    ...
    long_df["reference_date"] = pd.to_datetime(long_df["year"].astype(str) + "-07-01")
...
def ingest_pep_county(
    ...
    if series not in {AUTO_SERIES, POSTCENSAL_SERIES}:
        raise ValueError(
            f"Unknown series '{series}'. Expected one of: {AUTO_SERIES}, {POSTCENSAL_SERIES}."
        )
    ...
    logger.info(f"Parsed {len(df)} PEP records for {county_count} counties, years {year_range}")
